...
mod1_server <-function(input, output, session) {
  ...
  observeEvent(input$cf, {
    req(input$cf)
    shinyFeedback::feedbackWarning("cf", (input$cf > 0.5 & input$cf < 1) , text = "Contamination Factor is High" ,
  ...
  output$pairplot <- renderPlot({
    ...
    if (length(input$user_selected_columns) > 1) {
  ...
  fetchplot <- observeEvent(input$go, {
    ...
    updateTabsetPanel(session, "inPlotSet",
                      selected = "pcaplottab")
    waitress <- waiter::Waitress$new("#inPlotSet", theme = "overlay-percent")
    for(i in 1:10){
  ...
  train_isoforest_anomaly_plot <- function(df_input){
    ...
    pca_input  <- select_if(data$df, is.numeric)
